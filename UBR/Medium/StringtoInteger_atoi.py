...
class Solution:
    def myAtoi(self, s: str) -> int:
        ...
        if ('-' in res and len(res) == 1) or len(res) == 0: return 0
        if res:
            res = int(res)
            if res < -(2**31):
                return -(2**31)
        ...
        # if not s[pm:].isdigit():
        #     end_res = [not x.isdigit() for x in s[pm:]].index(True)
        #     # print([not x.isdigit() for x in s[pm:]], end_res)
        # if not s[pm].isdigit():
        #     res = 0
        # else: 
        #     res = int(s[pm:end_res+pm])
